			...
			if(localLightning == null && getWorld().rand.nextFloat() <= 0.01F) {
				...
				if(getWorld().getBlockState(dp).getBlock() == BlocksCore.mruCoil) {
					...
					if(tile.canWork()) {
						...
						localLightning = new Lightning(getWorld().rand, new Coord3D(0.5F+fDir.getXOffset()/2.3F, 0, 0.5F+fDir.getZOffset()/2.3F), new Coord3D(0.5F+fDir.getXOffset()/2.3F, 1, 0.5F+fDir.getZOffset()/2.3F), 0.03F, 1F, 0.1F, 0.8F);
